...
#define AHB_BASE_ADDR2 0x20080000UL   // Base real de la RAM AHB (32 KB)
...
myLLI_t *cfgLLI  = (myLLI_t *)AHB_BASE_ADDR2;
...
volatile uint32_t *bufferADC2  = (volatile uint32_t *)(AHB_BASE_ADDR2 + sizeof(myLLI_t)); // Buffer para datos ADC
...
int main(void) {
    ...
    while (1){
		...
		if (calibrated && buffer_ready_dma){
        buffer_ready_dma = 0;
        if(buffer_index == 0){
            GPDMA_ChannelCmd(0, DISABLE);
            GPDMA_ChannelCmd(1, ENABLE);

            // Cambiar el baudiaje del UART para enviar datos más rápido
                uint32_t PCLK_UART = 25000000; // Asumir PCLK = 25MHz
                uint32_t Fdiv = (PCLK_UART) / (16 * 921600);
            ...
            for(int i = 0; i < NUM_SAMPLES; i++) {
            // Extraemos los 12 bits de datos (0-4095)
            uint16_t sample = (uint16_t)((bufferADC2[i] >> 4) & 0xFFF);
...
void cfgDMA(void){
    ...
    cfgLLI2->NextLLI = (uint32_t)cfgLLI2; // apunta al propio descriptor (no &cfgLLI)
    ...
    cfgDMA.SrcConn = GPDMA_CONN_ADC;
    cfgDMA.DstConn = 0;
    cfgDMA.DMALLI = (uint32_t)cfgLLI2; // apunta al descriptor en AHB
